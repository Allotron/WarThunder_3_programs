...
class VoiceWorker(QThread):
    ...
    def listen_until_stop(self, source):
        ...
        full_text = []
        # Увеличиваем паузу, чтобы не обрывало на полуслове во время диктовки
        self.recognizer.pause_threshold = 2.0 
        
        while True:
            try:
                # Слушаем без лимита времени (пока пользователь не замолчит сам)
                ...
                try:
                    ...
                    if self.input_mode == 1: # Режим кнопки
                        if not self.manual_trigger:
                            ...
                            phrase_limit = 4 # Слушаем ровно 4 секунды
                            timeout_val = 3  # Ждем начала речи 3 секунды
                    ...
                    try:
                        ...
                        # Если тишина (пользователь нажал кнопку, но промолчал)
                        if self.input_mode == 1:
                            self.status_signal.emit(f"Цель: {self.target_lang.upper()}", "rgba(0,0,255,100)", 0)
                        continue 
                    ...
                    # --- 3. ПРОВЕРКА ТРИГГЕРА (только для режима "Голос") ---
                    clean_cmd = command
                    ...
                    # КОМАНДА: ПЕРЕВОД (Сообщение)
                    if "сообщен" in clean_cmd:
                        # Включаем режим бесконечной записи до стоп-слова
                    ...
                    # ДРУГИЕ КОМАНДЫ
                        ...
                        # Если ничего не поняли в режиме кнопки - просто сбрасываем статус
                        if not found_lang and self.input_mode == 1:
                            pass # Статус сбросится ниже

                    # Возврат статуса в норму (если это был режим кнопки)
                    if self.input_mode == 1:
                        # Небольшая задержка перед сбросом, чтобы юзер успел прочитать результат (если это не перевод)
                        QTimer.singleShot(1500, lambda: self.status_signal.emit(f"Цель: {self.target_lang.upper()}", "rgba(0,0,255,100)", 0))

                except sr.UnknownValueError:
                    # Звук был, но слов не разобрали
                    if self.input_mode == 1:
                        self.status_signal.emit(f"Цель: {self.target_lang.upper()}", "rgba(0,0,255,100)", 0)
...
class Overlay(QWidget):
    ...
    def update_geometry(self):
        self.setGeometry(self.config['overlay_x'], self.config['overlay_y'], 
    ...
    def reset_status(self):
        ...
        self.resize(self.config['overlay_w'], self.config.get('overlay_h', 300))
...
class Settings(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.sl_x = QSlider(Qt.Orientation.Horizontal); self.sl_x.setRange(0, 2560); self.sl_x.setValue(self.config['overlay_x'])
        self.sl_y = QSlider(Qt.Orientation.Horizontal); self.sl_y.setRange(0, 1440); self.sl_y.setValue(self.config['overlay_y'])
        ...
        form_pos.addRow("X:", self.sl_x)
        form_pos.addRow("Y:", self.sl_y)

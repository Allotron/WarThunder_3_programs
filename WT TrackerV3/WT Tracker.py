    ...
    def save_unknowns(self):
        ...
        try:
            ...
            with open(UNKNOWN_FILE, "w", encoding="utf-8") as f:
                f.write("// Скопируйте строки ниже в vehicles.json...\n")
                        ...
                        if p1_name: 
                            ...
                            if not is_recon_drone:
                                ...
                                if p2_name in self.active_enemies: 
                                    # --- ЗАПИСЬ НА КЛАДБИЩЕ ---
                                    # Запоминаем технику и время смерти
                                    self.dead_enemies[p2_name] = {
                                        "veh": self.active_enemies[p2_name]["veh"],
                                        "time": time.time()
                                    }
                    ...
                    if len(parts) > 0:
                        ...
                        if p_name:
                            ...
                            if p_name in self.active_enemies: 
                                # --- ЗАПИСЬ НА КЛАДБИЩЕ ПРИ СОЛО-СМЕРТИ ---
                                self.dead_enemies[p_name] = {
                                    "veh": self.active_enemies[p_name]["veh"],
                                    "time": time.time()
                                }

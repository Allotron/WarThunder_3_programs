...
from PyQt6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QFrame, QGridLayout, QMessageBox, QLineEdit)
...
class WTLogic:
    def __init__(self, sound_callback):
        ...
        # --- СПИСОК ТЕХНИКИ С ОДИНАКОВЫМ НАЗВАНИЕМ У РАЗНЫХ НАЦИЙ ---
        # Сюда вписывай части названий (в нижнем регистре), которые встречаются у нескольких стран.
        # Скрипт пометит их нацию как 'unknown', и они будут подсвечиваться красным,
        # если убили твоего союзника.
    ...
    def find_vehicle_info(self, log_name):
        ...
        # --- БЕЗОПАСНЫЙ ФИЛЬТР МУСОРА ---
        # 1. Фильтр калибров (ТОЛЬКО ЕСЛИ НАЧИНАЕТСЯ С НИХ)
        ...
        # 3. Фильтр ракет типа "9m133" (но не yak_9m)
        if re.match(r'^9m\d+', clean_name_lower):
             self.name_cache[log_name] = None
             return None

        # 4. РАЗВЕДЫВАТЕЛЬНЫЙ ДРОН (игнор)
        if "recon micro" in clean_name_lower:
            self.name_cache[log_name] = None
            return None
            
        # --- ЛОГИКА ОБЩЕЙ ТЕХНИКИ (КЛОНЫ) ---
        # Проверяем, содержится ли название в нашем списке shared_vehicles
        # Если да -> помечаем как unknown (чтобы работала логика определения по жертве)
        
        # Определяем тип техники по умолчанию (можно усложнить, если нужно)
        v_type = "tank" 
        if "mq-1" in clean_name_lower: v_type = "aircraft"
        
        for shared_name in self.shared_vehicles:
            if shared_name in clean_name_lower:
                # Особая пометка для дрона, для остальных просто название
        ...
        # 7. Если ничего не подошло — это новая техника
            ...
            for msg in msgs:
                ...
                if action_found:
                    ...
                    if len(parts) == 2:
                        ...
                        # Обработка AI дронов (у них нет имени игрока)
                        if not p2_veh:
                            p2_veh = p2_raw
                            p2_name = "AI_Drone"
                        ...
                        if p1_name: 
                            ...
                            # 1. Если убийца на MQ-1 (нация unknown)
                            if p1_veh_clean == "MQ-1":
                                # Мы смотрим на ЖЕРТВУ. 
                                # Если жертва НЕ входит в список выбранных нами врагов (значит жертва - свой),
                                # то убийца (MQ-1) должен быть помечен как ВРАГ.
                                if victim_info:
                                    victim_nation = victim_info.get('nation')
                                    # Если нация жертвы известна и она НЕ в списке врагов -> значит убили нашего -> MQ-1 враг
                                    if victim_nation and victim_nation not in self.selected_nations:
                                        is_enemy = True
                            
                            # 2. Если убийца на обычной технике (нация известна)
                            elif killer_info:
                            ...
                            # --- ЛОГИКА ЖЕРТВЫ ---
                            # Recon Micro не считаем за смерть и не удаляем из списка
                            ...
                            if not is_recon_drone:
                                ...
                                # Удаляем проигравшего из оверлея
                ...
                if solo_found:
                    ...
                    if len(parts) > 0:
                        ...
                        if p_name:
                            ...
                            if p_name in self.active_enemies: del self.active_enemies[p_name]
                if mid > max_id: max_id = mid
...
# --- НОВЫЙ ПОТОК ОПРОСА КЛАВИШ (ДЛЯ МУЛЬТИНАЖАТИЯ) ---
class KeyPollingWorker(QThread):
    ...
    def run(self):
        ...
        while self.running:
            try:
                # 1. Проверка кнопки СКАНИРОВАНИЯ
                ...
                time.sleep(0.05) # 20 раз в секунду - достаточно быстро и не грузит CPU
...
class Overlay(QWidget):
    ...
    def is_visible_by_filter(self, nation, v_type):
        # Если это ударный дрон (MQ-1), показываем его независимо от выбранной нации
        ...
        elif "aircraft" in v_type: type_key = "plane" # Для MQ-1
    ...
    def update_view(self):
        ...
        if self.temp_test_mode:
            ...
            # Словарь иконок для типов техники
            type_icons = {
                "tank": "🛡️",      # Танк
                "plane": "✈️",     # Самолет
                "heli": "🚁",      # Вертолет
                "zsu": "📡",       # Зенитка (ЗСУ)
                "special": "⭐"    # Особое/Неизвестное
            }
            ...
            for name, data in current_enemies.items():
                ...
                if is_nuke:
                    # Если ядерка — подсвечиваем синим и добавляем значок радиации
                    line = f"<span style='color:#00FFFF'>☢️ {display_text} [{player_stats.kills} K]</span>"
                else:
                    # Обычный враг — красный цвет и иконка техники
...
class Settings(QWidget):
    # Сигналы больше не нужны для хуков, но нужны для кнопок
    ...
    def __init__(self):
        ...
        self.hooks = {} # Только для Setup F6/F7
        ...
        # Инициализация Worker'а для клавиш
    ...
    # ОБНОВЛЕНИЕ КЛАВИШ ТЕПЕРЬ ПРОСТО ПЕРЕДАЕТ ИХ В WORKER
    ...
    def enable_setup_hotkeys(self):
        if not keyboard: return
        try:
            # F6 и F7 оставим на хуках, так как это настройка, а не игровой процесс
    ...
    def init_ui(self):
        ...
        self.slider_x = self.create_slider(sl_layout, "Позиция X", 0, screen.width(), self.update_ov_preview)
        self.slider_y = self.create_slider(sl_layout, "Позиция Y", 0, screen.height(), self.update_ov_preview)
        ...
        scan_info = QLabel("<b>Настройка зоны(Клавиши F6 и F7 функционируют только при активированном меню настроек оверлея):</b><br>1. Курсор в левый-верх угол -> <b>F6</b><br>2. Курсор в правый-нижний -> <b>F7</b>")
    ...
    def update_scan_hotkey_from_ui(self):
        ...
        self.update_keys_in_worker() # ОБНОВЛЯЕМ В WORKER
        self.save_config() 
    ...
    def update_overlay_hotkey_from_ui(self):
        ...
        self.update_keys_in_worker() # ОБНОВЛЯЕМ В WORKER
        self.save_config()
        self.update_ov_status_label() # Обновить лейбл, чтобы показать новую кнопку
